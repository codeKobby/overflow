...
Every command must work when explicitly invoked and must degrade selectable questions to numbered or lettered text. Orchestration commands such as `/setup-learning`, `/quiz`, `/teach`, and `/learn` are user-invoked by default. Do not depend on a host-specific question tool for correctness.
