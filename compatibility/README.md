...
Overflow may inspect standard `SKILL.md` metadata in project or global skill roots to propose a specialist handoff. The handoff must show the matching skill, reason, possible file or command effects, and request confirmation. If the host cannot invoke another skill programmatically, it must show the equivalent explicit invocation or text choices. The specialist must not silently mutate `.learning/`, learner records, branches, or external systems. Afterward, Overflow should verify observed changes before assessment or progress updates.
