...
All Tier A hosts consume the same portable `SKILL.md` body. The `agents/openai.yaml` files only provide Codex picker metadata and do not change the portable instructions. `/overflow` is a route-first entrypoint: it should announce the selected route, run `/setup-learning` when a stateful request needs missing initialization, preserve the original request, and continue after setup confirmation.
...
The learner must be able to run `/overflow`, `/help`, `/setup-learning`, `/teach`, `/quiz`, `/exercise`, `/hint`, `/assess`, `/learn`, and `/handoff`, or the host’s equivalent explicit skill command. Before stateful learning commands, the host must preserve Overflow’s readiness check for missing, draft, partial, initialized, or invalid `.learning/` state. Missing or incomplete state should produce an announced `/setup-learning` route with continuation of the original request, not a generic internal-instruction prompt. A one-off stateless path remains available for meaningful commands such as `/explain`. Selectable questions must degrade to numbered or lettered text. Relative references must resolve. `.learning/` must remain repository-local and separate from host configuration. The host adapter must not weaken learner confirmation, source-citation, privacy, or Python cybersecurity boundaries.

When Git is available, every host must preserve the optional setup choice between a local exercise branch, a linked worktree, the current branch, and deciding later. It must show dirty paths and the exact branch/worktree proposal before creation, preserve `.learning/git-workflow.json`, and keep staging, committing, pushing, opening a draft pull request, merging, deletion, cleanup, and branch switching as separate confirmed actions. A host must not silently run the Git helper with `--apply`.

Overflow may inspect standard `SKILL.md` metadata in project or global skill roots to propose a specialist handoff. The route announcement must show the matching skill, reason, possible file or command effects, and return point. Only model-invoked specialists may be automatically invoked when the host supports that policy; user-invoked skills must be offered as explicit commands. If the host cannot invoke another skill programmatically, it must show the equivalent explicit invocation or text choices. The specialist must not silently mutate `.learning/`, learner records, branches, or external systems. Afterward, Overflow should verify observed changes before assessment or progress updates.
