...
1. Read the source prompt, relevant lesson, starter files, hints, solution policy, repository standards, tests, documented commands, and the selected native-or-inferred evidence map. Recognize Practice, Prove it, Finish line, self-assessment, verification, hints, solutions, and safety/scope by heading or linked artifact, not fixed filenames. Cache only path metadata and source hashes.
...
4. Write `.learning/exercises/<exercise-id>/manifest.json` with the prompt source path, source hash, question IDs, source anchors, target files, answer regions, acceptance criteria, approved checks, active question, hint level, status, and an `evidence_plan` linking each question to native or inferred proof prompts, verification checks, finish-line gates, reflection, and safety sections.
...
4. **Assess.** `/assess` resolves the active marker, inspects the prompt and changed region, runs only approved checks after permission, and writes implementation evidence.
...
6. **Finish and advance.** Ask unresolved native or inferred Finish line and self-assessment gates, then update the question status and move `active_question` to the next unanswered or needs-revision question. Tell the learner what is active now.
